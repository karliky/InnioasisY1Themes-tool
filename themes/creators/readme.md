...
- **`menuItemBackground`** - Background image for unselected menu items (submenu/list rows)
  - Type: image file path (PNG) or color string like `#RRGGBB`/`#AARRGGBB`
  - Where it's used: submenu/list item rows in file lists, settings submenus, video lists
  - How to get there: Home → Music → list of songs/playlists; Home → Video → list of videos; Home → Settings → any submenu list
  - Handled by: `ThemeManager.menuItemSetBackground()` (jadx-out/sources/com/innioasis/y1/theme/ThemeManager.java)
- **`menuItemTextColor`** - Text color for unselected menu items
- **`menuItemSelectedBackground`** - Background image for selected menu item (submenu/list rows)
  - Type: image file path or color string
  - Where it's used: same list rows as above, when the item is selected/highlighted
  - How to get there: same locations as menuItemBackground
  - Handled by: `ThemeManager.menuItemSetBackground()` (same handler)
...
- **`folderIcon`** - Icon for folders in file browser
  - Type: image file path (PNG)
  - Where it's used: file list row icon for folder items
  - How to get there: Home → Music → Local Files / File browser (also used in video/ebook file lists)
  - Handled by: `ThemeManager.commonSetIcon()` (used by file list adapters)
- **`musicIcon`** - Icon for music file items in file browsers
  - Type: image file path (PNG)
  - Where it's used: file list row icon for music items
  - How to get there: Home → Music → Local Files / File browser
  - Handled by: `ThemeManager.commonSetIcon()` (used by file list adapters)
...
- **`progressColor`** - Foreground/progress fill color for player progress bars
  - Type: color string `#RRGGBB`/`#AARRGGBB`
  - Where it's used: music player progress bar (the filled portion)
  - How to get there: Home → Music → Now Playing / start a track
  - Handled by: `ThemeManager` progress bar color handlers
- **`progressBackgroundColor`** - Background/track color for player progress bars
  - Type: color string
  - Where it's used: music player progress bar (the empty portion)
  - How to get there: Home → Music → Now Playing / start a track
  - Handled by: `ThemeManager` progress bar color handlers

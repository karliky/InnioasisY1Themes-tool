...
import { loadAvailableThemes, loadClonedThemes, cloneTheme, updateClonedThemeAsset, deleteClonedTheme, updateClonedThemeSpec } from './services/themeService';
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-10 no-scrollbar relative z-10">
            ...
            {availableThemes.length > 0 && (
              <>
                ...
                {availableThemes.some(t => t.isEditable) && (
                  <section className="space-y-3 editorial-section">
                    <h3 className="text-[11px] font-bold text-[#D4A574] uppercase tracking-[0.15em] border-b-2 border-[#6B7A47] pb-2 flex items-center gap-2" style={{ fontFamily: 'var(--font-mono)' }}>
                      ...
                      My Cloned Themes (Editable)
                    </h3>
                    <div className="flex flex-col gap-2.5 pl-2">
                      {availableThemes.filter(t => t.isEditable).map(t => (
                        <div key={t.id} className="flex gap-2">
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span>{(t as any)?.spec?.theme_info?.title || t.id.replace(/_clone_\d+$/, '')}</span>
                            </div>
                            {t.originalThemeId && (
